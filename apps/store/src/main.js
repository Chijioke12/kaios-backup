...
var systemAppCache = null; // High-performance system-wide application cache
...
function refreshSystemAppsCache() {
	return new Promise(function(resolve) {
		if (!navigator.mozApps || !navigator.mozApps.mgmt) {
			var virtualStr = localStorage.getItem('virtual_installed_apps') || '{}';
			var parsed = JSON.parse(virtualStr);
			systemAppCache = [];
			for (var key in parsed) {
				if (parsed.hasOwnProperty(key)) {
					systemAppCache.push(parsed[key]);
				}
			}
			resolve(systemAppCache);
			return;
		}
		
		var request = navigator.mozApps.mgmt.getAll();
		request.onsuccess = function() {
			systemAppCache = this.result || [];
			resolve(systemAppCache);
		};
		request.onerror = function() {
			systemAppCache = [];
...
function checkAppStatus(app) {
	return new Promise(function(resolve) {
		var executeCheck = function() {
			var installedApps = systemAppCache || [];
			var localApp = null;
		...
		if (systemAppCache === null) {
			refreshSystemAppsCache().then(executeCheck);
		} else {
			executeCheck();
		}
	});
...
function refreshAppStatusMultipleTimes(app) {
	var checkAndUpdate = function() {
		refreshSystemAppsCache().then(function() {
...
function initApp() {
	...
	if (navigator.mozApps && navigator.mozApps.mgmt) {
		navigator.mozApps.mgmt.oninstall = function(event) {
			console.log("System-wide app installed event received:", (event && event.application && event.application.manifestURL));
			refreshSystemAppsCache().then(function() {
		...
		navigator.mozApps.mgmt.onuninstall = function(event) {
			console.log("System-wide app uninstalled event received:", (event && event.application && event.application.manifestURL));
			refreshSystemAppsCache().then(function() {
				if (currentView === 'details' && cachedApps[currentAppIndex]) {
					refreshAppStatusMultipleTimes(cachedApps[currentAppIndex]);
				}
	...
	// Priming system-wide caches first before loading the registry to lock high UI speed
	refreshSystemAppsCache().then(function() {
		loadRegistry();
	});
}
